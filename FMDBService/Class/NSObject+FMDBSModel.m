...
#define FMDBSImage UIImage
#define FMDBSColor UIColor
#else
#import <AppKit/AppKit.h>
#define FMDBSImage NSImage
#define FMDBSColor NSColor
#endif

static char FMDBSModelBase_Key_RowID;
static char FMDBSModelBase_Key_TableName;
static char FMDBSModelBase_Key_Inserting;

@interface FMDBService (FMDBService_FMDBSModel)
+ (BOOL)nullIsEmpty;
@end

@implementation NSObject (FMDBSModel)

+ (FMDBService *)getUsingFMDBService
{
    ///ios8 能获取系统类的属性了  所以没有办法判断属性数量来区分自定义类和系统类
    ///可能系统类的存取会不正确
    static FMDBService *helper;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        helper = [[FMDBService alloc] init];
    });
    return helper;
}
...
{
    objc_setAssociatedObject(self, &FMDBSModelBase_Key_RowID, [NSNumber numberWithInteger:rowid], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSInteger)rowid
{
    return [objc_getAssociatedObject(self, &FMDBSModelBase_Key_RowID) integerValue];
}

- (void)setDb_tableName:(NSString *)db_tableName
{
    objc_setAssociatedObject(self, &FMDBSModelBase_Key_TableName, db_tableName, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
- (NSString *)db_tableName
{
    NSString *tableName = objc_getAssociatedObject(self, &FMDBSModelBase_Key_TableName);
...
- (BOOL)db_inserting
{
    return [objc_getAssociatedObject(self, &FMDBSModelBase_Key_Inserting) boolValue];
}
- (void)setDb_inserting:(BOOL)db_inserting
{
    ...
    objc_setAssociatedObject(self, &FMDBSModelBase_Key_Inserting, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
#pragma 无关紧要的
+ (NSString *)getDBImagePathWithName:(NSString *)filename
{
    NSString *dir = [NSString stringWithFormat:@"dbimg/%@", NSStringFromClass(self)];
    return [FMDBSUtils getPathForDocuments:filename inDir:dir];
}
...
///get
- (id)modelGetValue:(FMDBSProperty *)property
{
    ...
    if (value == nil) {
        ...
        returnValue = [[FMDBSUtils numberFormatter] stringFromNumber:value];
    }
        ...
        if (formatter) {
            ...
            returnValue = [FMDBSUtils stringWithDate:value];
        }
        returnValue = [returnValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    }
    else if ([value isKindOfClass:[FMDBSColor class]]) {
        FMDBSColor *color = value;
        CGFloat r, g, b, a;
    ...
    else if ([value isKindOfClass:[FMDBSImage class]]) {
...
///set
- (void)modelSetValue:(FMDBSProperty *)property value:(NSString *)value
{
    ///参试获取属性的Class
    ...
    if (columnClass == nil) {
        ...
        NSString *columnType = property.propertyType;
        if ([FMSQL_Convert_FloatType rangeOfString:columnType].location != NSNotFound) {
            if (value) {
                modelValue = [[FMDBSUtils numberFormatter] numberFromString:value];
            }
        }
        else if ([FMSQL_Convert_IntType rangeOfString:columnType].location != NSNotFound) {
            if (value) {
                modelValue = [[FMDBSUtils numberFormatter] numberFromString:value];
            }
        }
        ...
        if (![FMDBService nullIsEmpty] || value.length > 0) {
        ...
        if (formatter) {
            ...
            modelValue = [FMDBSUtils dateWithString:datestr];
        }
    }
    else if ([columnClass isSubclassOfClass:[FMDBSColor class]]) {
        ...
        modelValue = [FMDBSColor colorWithRed:r green:g blue:b alpha:a];
    }
    else if ([columnClass isSubclassOfClass:[FMDBSImage class]]) {
        NSString *filename = value;
        NSString *filepath = [self.class getDBImagePathWithName:filename];
        if ([FMDBSUtils isFileExists:filepath]) {
            modelValue = [[FMDBSImage alloc] initWithContentsOfFile:filepath];
        }
    }
...
- (id)db_jsonObjectFromDictionary:(NSDictionary *)dic
{
    if ([NSJSONSerialization isValidJSONObject:dic]) {
        NSDictionary *bomb = @{ FMDBS_TypeKey : FMDBS_TypeKey_JSON, FMDBS_ValueKey : dic };
        return bomb;
    }
    else {
        ...
        if (toDic.count) {
            NSDictionary *bomb = @{ FMDBS_TypeKey : FMDBS_TypeKey_Combo, FMDBS_ValueKey : toDic };
            return bomb;
        }
    }
    return nil;
...
- (id)db_jsonObjectFromArray:(NSArray *)array
{
    if ([NSJSONSerialization isValidJSONObject:array]) {
        NSDictionary *bomb = @{ FMDBS_TypeKey : FMDBS_TypeKey_JSON, FMDBS_ValueKey : array };
        return bomb;
    }
    else {
        ...
        if (toArray.count) {
            NSDictionary *bomb = @{ FMDBS_TypeKey : FMDBS_TypeKey_Combo, FMDBS_ValueKey : toArray };
            return bomb;
        }
    }
    return nil;
...
- (id)db_jsonObjectWithObject:(id)obj
{
    ...
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSNumber class]]) {
        ...
        if (formatter) {
            ...
            dateString = [FMDBSUtils stringWithDate:obj];
        }
        dateString = [dateString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        if (dateString.length > 0) {
            jsonObject = @{ FMDBS_TypeKey : FMDBS_TypeKey_Date, FMDBS_ValueKey : dateString };
        }
    }
...
- (id)db_jsonObjectFromModel:(NSObject *)model
{
    ...
    if (model.rowid > 0) {
        ...
        if (model.db_inserting == NO && [clazz getModelInfos].count > 0) {
            ...
            NSAssert(NO, @"目前FMDBS 还不支持 循环引用。  比如 A 持有 B，   B 持有 A，这种的存储");
        }
    }
    return jsonObject;
}
- (NSDictionary *)db_readInfoWithModel:(NSObject *)model class:(Class)clazz
{
    ...
    jsonObject[FMDBS_TypeKey] = FMDBS_TypeKey_Model;
    jsonObject[FMDBS_TableNameKey] = model.db_tableName;
    jsonObject[FMDBS_ClassKey] = NSStringFromClass(clazz);
    jsonObject[FMDBS_RowIdKey] = @(model.rowid);
    
    NSDictionary *dic = [model db_getPrimaryKeysValues];
    if (dic.count > 0 && [NSJSONSerialization isValidJSONObject:dic]) {
        jsonObject[FMDBS_PValueKey] = dic;
    }
    return jsonObject;
}
...
- (id)db_objectWithDictionary:(NSDictionary *)dic
{
    ...
    NSString *type = [dic objectForKey:FMDBS_TypeKey];
    if (type) {
        if ([type isEqualToString:FMDBS_TypeKey_Model]) {
            Class clazz = NSClassFromString([dic objectForKey:FMDBS_ClassKey]);
            NSInteger rowid = [[dic objectForKey:FMDBS_RowIdKey] integerValue];
            NSString *tableName = [dic objectForKey:FMDBS_TableNameKey];

            NSString *where = nil;

            NSString *rowCountWhere = [NSString stringWithFormat:@"select count(rowid) from %@ where rowid=%ld limit 1", tableName, (long)rowid];
            NSInteger result = [[[clazz getUsingFMDBService] executeScalarWithSQL:rowCountWhere arguments:nil] integerValue];
            if (result > 0) {
                where = [NSString stringWithFormat:@"select rowid,* from %@ where rowid=%ld limit 1", tableName, (long)rowid];
            }
            else {
                NSDictionary *pv = [dic objectForKey:FMDBS_PValueKey];
                if (pv.count > 0) {
            ...
            if (where) {
                NSArray *array = [[clazz getUsingFMDBService] searchWithSQL:where toClass:clazz];
                if (array.count > 0) {
        ...
        else if ([type isEqualToString:FMDBS_TypeKey_JSON]) {
            id value = [dic objectForKey:FMDBS_ValueKey];
            return value;
        }
        else if ([type isEqualToString:FMDBS_TypeKey_Combo]) {
            id value = [dic objectForKey:FMDBS_ValueKey];
            if ([value isKindOfClass:[NSArray class]]) {
        ...
        else if ([type isEqualToString:FMDBS_TypeKey_Date]) {
            NSString *datestr = [dic objectForKey:FMDBS_ValueKey];
            NSDateFormatter *formatter = [self.class getModelDateFormatter];
            if (formatter) {
                return [formatter dateFromString:datestr];
            }
            else {
                return [FMDBSUtils dateWithString:datestr];
            }
        }
    }
    else {
...
#pragma mark -
- (void)userSetValueForModel:(FMDBSProperty *)property value:(id)value
{
}
- (id)userGetValueForModel:(FMDBSProperty *)property
{
    return nil;
}
...
{
    FMDBSModelInfos *infos = [self.class getModelInfos];
    ...
    [array enumerateObjectsUsingBlock:^(NSString *pname, NSUInteger idx, BOOL * _Nonnull stop) {
        FMDBSProperty *property = [infos objectWithSqlColumnName:pname];
        id value = nil;
        if ([property.type isEqualToString:FMSQL_Mapping_UserCalculate]) {
...
{
    FMDBSProperty *property = [self singlePrimaryKeyProperty];
    if (property) {
        id pkvalue = [self singlePrimaryKeyValue];
        if ([property.sqlColumnType isEqualToString:FMSQL_Type_Int]) {
            if ([pkvalue isKindOfClass:[NSString class]]) {
                if ([FMDBSUtils checkStringIsEmpty:pkvalue])
                    return YES;
...
- (FMDBSProperty *)singlePrimaryKeyProperty
{
    FMDBSModelInfos *infos = [self.class getModelInfos];
...
+ (NSString *)db_rowidAliasName
{
    ...
    if (infos.primaryKeys.count == 1) {
        ...
        if ([primaryType isEqualToString:FMSQL_Type_Int]) {
...
+ (FMDBSModelInfos *)getModelInfos
{
    ...
    if (infos == nil) {
        ...
        if ([self isContainParent] && [self superclass] != [NSObject class]) {
            FMDBSModelInfos *superInfos = [[self superclass] getModelInfos];
            for (NSInteger i = 0; i < superInfos.count; i++) {
                FMDBSProperty *db_p = [superInfos objectWithIndex:i];
        ...
        if (pronames.count > 0) {
            infos = [[FMDBSModelInfos alloc] initWithKeyMapping:keymapping propertyNames:pronames propertyType:protypes primaryKeys:pkArray];
        }
        else {
            infos = [[FMDBSModelInfos alloc] init];
        }
...
+ (void)getSelfPropertys:(NSMutableArray *)pronames protypes:(NSMutableArray *)protypes
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([FMDBSUtils checkStringIsEmpty:propertyClassName]) {
            ///没找到具体的属性就放弃
            continue;
        }
        ///添加属性
