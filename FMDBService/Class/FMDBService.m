...
#define FMDBSCheck_tableNameIsInvalid(tableName)                           \
    if ([FMDBSUtils checkStringIsEmpty:tableName]) {                       \
        FMErrorLog(@" \n Fail!Fail!Fail!Fail! \n with TableName is nil"); \
        return NO;                                                        \
    }

#define FMDBSCode_Async_Begin             \
    __FMDBSWeak FMDBService *wself = self; \
    [self asyncBlock :^{__strong FMDBService *sself = wself;           \
                        if (sself) {

#define FMDBSCode_Async_End \
    }                      \
    }];

#define FMDBSCheck_modelIsInvalid(model)                                                            \
    if (model == nil) {                                                                            \
        FMErrorLog(@"model is nil");                                                               \
        return NO;                                                                                 \
    }                                                                                              \
    if ([model.class getModelInfos].count == 0) {                                                  \
        FMErrorLog(@"class: %@  property count is 0!!", NSStringFromClass(model.class));           \
        return NO;                                                                                 \
    }                                                                                              \
    NSString *_model_tableName = model.db_tableName ?: [model.class getTableName];                 \
    if ([FMDBSUtils checkStringIsEmpty:_model_tableName]) {                                         \
        FMErrorLog(@"model class name %@ table name is invalid!", NSStringFromClass(model.class)); \
        return NO;                                                                                 \
    }

@interface NSObject (FMTabelStructure_Private)
- (void)setDb_inserting:(BOOL)db_inserting;
@end

@interface FMDBSWeakObject : NSObject
@property (nonatomic, FMDBSWeak) FMDBService *obj;
@end

@interface FMDBService ()
@property (nonatomic, FMDBSWeak) FMDatabase *usingdb;
...
static BOOL FMDBSLogErrorEnable = NO;
+ (void)setLogError:(BOOL)logError
{
    if (FMDBSLogErrorEnable == logError) {
        return;
    }
#ifdef DEBUG
    FMDBSLogErrorEnable = logError;
    NSMutableArray *dbArray = [self dbHelperSingleArray];
    @synchronized(dbArray)
    {
        [dbArray enumerateObjectsUsingBlock:^(FMDBSWeakObject *weakObj, NSUInteger idx, BOOL *stop) {
            [weakObj.obj executeDB:^(FMDatabase *db) {
                db.logsErrors = FMDBSLogErrorEnable;
            }];
        }];
    }
#endif
}
static BOOL FMDBSNullIsEmptyString = NO;
+ (void)setNullToEmpty:(BOOL)empty
{
    FMDBSNullIsEmptyString = empty;
}
+ (BOOL)nullIsEmpty
{
    return FMDBSNullIsEmptyString;
}
...
+ (FMDBService *)dbHelperWithPath:(NSString *)dbFilePath save:(FMDBService *)helper
{
    NSMutableArray *dbArray = [self dbHelperSingleArray];
    FMDBService *instance = nil;
    @synchronized(dbArray)
    {
        if (helper) {
            FMDBSWeakObject *weakObj = [[FMDBSWeakObject alloc] init];
            weakObj.obj = helper;
            [dbArray addObject:weakObj];
        }
        else if (dbFilePath) {
            for (NSInteger i = 0; i < dbArray.count;) {
                FMDBSWeakObject *weakObj = [dbArray objectAtIndex:i];
                if (weakObj.obj == nil) {
...
- (instancetype)init
{
    return [self initWithDBName:@"enccaches"];
}

- (instancetype)initWithDBName:(NSString *)dbname
{
    return [self initWithDBPath:[FMDBService getDBPathWithDBName:dbname]];
}

- (instancetype)initWithDBPath:(NSString *)filePath
{
    if ([FMDBSUtils checkStringIsEmpty:filePath]) {
        ///release self
        self = nil;
        return nil;
    }

    FMDBService *helper = [FMDBService dbHelperWithPath:filePath save:nil];
    ...
    if (helper) {
        ...
        if (self) {
            ...
            [FMDBService dbHelperWithPath:nil save:self];
        }
    }
...
+ (NSString *)getDBPathWithDBName:(NSString *)dbName
{
    ...
    NSString *filePath = [FMDBSUtils getPathForDocuments:fileName inDir:@"db"];
    return filePath;
}

- (void)setDBName:(NSString *)dbName
{
    [self setDBPath:[FMDBService getDBPathWithDBName:dbName]];
}
            ...
            BOOL success = [fileManager createDirectoryAtPath:dirPath
                                  withIntermediateDirectories:YES
                                                   attributes:attributes
                                                        error:&error];
            if (success == NO) {
                FMErrorLog(@"create dir error: %@", error.debugDescription);
            }
        }
        else {
            /**
...
- (void)executeForTransaction:(BOOL (^)(FMDBService *))block
{
    FMDBService *helper = self;
...
- (NSMutableArray *)extractQuery:(NSMutableString *)query where:(id)where
{
    ...
    if ([where isKindOfClass:[NSString class]] && ([FMDBSUtils checkStringIsEmpty:where] == NO)) {
...
{
    FMDBSModelInfos *infos = [model.class getModelInfos];
    ...
    if (primaryKeys.count > 0) {
        for (NSInteger i = 0; i < primaryKeys.count; i++) {
            ...
            if ([FMDBSUtils checkStringIsEmpty:pk] == NO) {
                FMDBSProperty *property = [infos objectWithSqlColumnName:pk];
                id pvalue = nil;

                if (property && [property.type isEqualToString:FMSQL_Mapping_UserCalculate]) {
...
- (void)dealloc
{
    NSArray *array = [FMDBService dbHelperSingleArray];
    @synchronized(array)
    {
        for (FMDBSWeakObject *weakObject in array) {
...
@end
@implementation FMDBService (DatabaseManager)
...
{
    FMDBSCheck_tableNameIsInvalid(tableName);
...
{
    [self executeDB:^(FMDatabase *db) {
        FMDBSModelInfos *infos = [clazz getModelInfos];
        ...
        for (NSInteger i = 0; i < infos.count; i++) {
            ...
            if ([columnArray containsObject:property.sqlColumnName.lowercaseString] == NO) {
                ...
                if ((property.length > 0) && [property.sqlColumnType isEqualToString:FMSQL_Type_Text]) {
                    [addColumePars appendFormat:@"(%ld)", (long)property.length];
                }

                if (property.isNotNull) {
                    [addColumePars appendFormat:@" %@", FMSQL_Attribute_NotNull];
                }

                if (property.checkValue) {
                    [addColumePars appendFormat:@" %@(%@)", FMSQL_Attribute_Check, property.checkValue];
                }

                if (property.defaultValue) {
                    [addColumePars appendFormat:@" %@ %@", FMSQL_Attribute_Default, property.defaultValue];
                }
                NSString *alertSQL = [NSString stringWithFormat:@"alter table %@ add column %@", tableName, addColumePars];
                NSString *defaultValue = @"0";
                if ([property.sqlColumnType isEqualToString:FMSQL_Type_Text]) {
                    if (FMDBSNullIsEmptyString) {
                        defaultValue = @"";
                    }
                    else {
                        defaultValue = @"null";
...
- (BOOL)_createTableWithModelClass:(Class)modelClass tableName:(NSString *)tableName
{
    ...
    FMDBSModelInfos *infos = [modelClass getModelInfos];

    if (infos.count == 0) {
        FMErrorLog(@"Class: %@ 0属性 不需要创建表", NSStringFromClass(modelClass));
        return NO;
    }
    ...
    for (NSInteger i = 0; i < infos.count; i++) {
        ...
        if (property.isNotNull) {
            [table_pars appendFormat:@" %@", FMSQL_Attribute_NotNull];
        }

        if (property.isUnique) {
            [table_pars appendFormat:@" %@", FMSQL_Attribute_Unique];
        }

        if (property.checkValue) {
            [table_pars appendFormat:@" %@(%@)", FMSQL_Attribute_Check, property.checkValue];
        }

        if (property.defaultValue) {
            [table_pars appendFormat:@" %@ %@", FMSQL_Attribute_Default, property.defaultValue];
        }
...
@implementation FMDBService (DatabaseExecute)

- (id)modelValueWithProperty:(FMDBSProperty *)property model:(NSObject *)model
{
    id value = nil;
...
- (void)search:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy offset:(NSInteger)offset count:(NSInteger)count callback:(void (^)(NSMutableArray *))block
{
    ...
    FMDBSCode_Async_Begin;
    FMDBSQueryParams *params = [[FMDBSQueryParams alloc] init];
...
- (NSMutableArray *)searchBaseWithParams:(FMDBSQueryParams *)params
{
    if (params.toClass == nil) {
        FMErrorLog(@"you search pars:%@! \n toClass is nil", params.getAllPropertysString);
        return nil;
    }

    NSString *db_tableName = params.tableName;

    if ([FMDBSUtils checkStringIsEmpty:db_tableName]) {
    ...
    else if ([FMDBSUtils checkStringIsEmpty:params.columns] == NO) {
    ...
    else if ([FMDBSUtils checkStringIsEmpty:params.where] == NO) {
...
- (NSMutableArray *)searchWithParams:(FMDBSQueryParams *)params
{
    if (params.callback) {
        FMDBSCode_Async_Begin;
        NSMutableArray *array = [sself searchBaseWithParams:params];
...
{
    if ([FMDBSUtils checkStringIsEmpty:groupBy] == NO) {
        [sql appendFormat:@" group by %@", groupBy];
    }

    if ([FMDBSUtils checkStringIsEmpty:orderby] == NO) {
    ...
    if (!modelClass) {
        ...
        while ([set next]) {
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                FMDBSProperty *property = [infos objectWithSqlColumnName:sqlName];
                ...
                if (isRowid && ((property == nil) || [property.sqlColumnType isEqualToString:FMSQL_Type_Int])) {
                    bindingModel.rowid = [set longForColumnIndex:i];
                }
                else {
                    BOOL isUserCalculate = [property.type isEqualToString:FMSQL_Mapping_UserCalculate];
...
{
    FMDBSCheck_modelIsInvalid(model);

    Class modelClass = model.class;

    // callback
    if ([modelClass dbWillInsert:model] == NO) {
        FMErrorLog(@"your cancel %@ insert", model);
        return NO;
    }
    ...
    FMDBSProperty *primaryProperty = [model singlePrimaryKeyProperty];

    for (NSInteger i = 0; i < infos.count; i++) {
        FMDBSProperty *property = [infos objectWithIndex:i];

        if ([FMDBSUtils checkStringIsEmpty:property.sqlColumnName]) {
            continue;
        }

        if ([property isEqual:primaryProperty]) {
            if ([property.sqlColumnType isEqualToString:FMSQL_Type_Int] && [model singlePrimaryKeyValueIsEmpty]) {
                continue;
            }
        }
...
- (BOOL)updateToDBBase:(NSObject *)model where:(id)where
{
    ...
    if ([modelClass dbWillUpdate:model] == NO) {
        FMErrorLog(@"you cancel %@ update.", model);
        return NO;
    }
    ...
    if ([where isKindOfClass:[NSString class]] && ([FMDBSUtils checkStringIsEmpty:where] == NO)) {
        ...
        if (pwhere.length == 0) {
            FMErrorLog(@"database update fail : %@ no find primary key!", NSStringFromClass(modelClass));
            return NO;
        }
...
- (BOOL)deleteToDBBase:(NSObject *)model
{
    ...
    if ([modelClass dbWillDelete:model] == NO) {
        FMErrorLog(@"you cancel %@ delete", model);
        return NO;
    }
    ...
    if (model.rowid > 0) {
        ...
        if (pwhere.length == 0) {
            FMErrorLog(@"delete fail : %@ primary value is nil", NSStringFromClass(modelClass));
            return NO;
        }
...
- (BOOL)isExistsModel:(NSObject *)model
{
    ...
    if (pwhere.length == 0) {
        FMErrorLog(@"exists model fail: primary key is nil or invalid");
        return NO;
    }
...
{
    [[modelClass getUsingFMDBService] executeDB:^(FMDatabase *db) {
...
#define FMTestDirFilename @"FMTestDirFilename111"
+ (void)clearFileWithTable:(Class)modelClass columns:(NSArray *)columns type:(NSInteger)type
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        switch (type) {
        case 1:
            testpath = [modelClass getDBImagePathWithName:FMTestDirFilename];
            break;

        case 2:
            testpath = [modelClass getDBDataPathWithName:FMTestDirFilename];
            break;
        }

        if ([FMDBSUtils checkStringIsEmpty:testpath]) {
            return;
        }

        NSString *dir = [testpath stringByReplacingOccurrencesOfString:FMTestDirFilename withString:@""];

        NSUInteger count = columns.count;

        // 获取该目录下所有文件名
        NSArray *files = [FMDBSUtils getFilenamesWithDir:dir];
        ...
        [[modelClass getUsingFMDBService] executeDB:^(FMDatabase *db) {
            ...
            while ([set next]) {
                for (int j = 0; j < count; j++) {
                    ...
                    if ([FMDBSUtils checkStringIsEmpty:str] == NO) {
        ...
        for (NSString *deletefile in files) {
            if ([dbfiles indexOfObject:deletefile] == NSNotFound) {
                [FMDBSUtils deleteWithFilepath:[dir stringByAppendingPathComponent:deletefile]];
            }
        }
    });
}
...
@implementation FMDBService (Deprecated_Nonfunctional)
- (void)setEncryptionKey:(NSString *)encryptionKey
{
    [self setKey:encryptionKey];
}
+ (FMDBService *)sharedDBHelper
{
    return [FMDBService getUsingFMDBService];
}
...
{
    return [[FMDBService alloc] init];
}
@end

@implementation FMDBSWeakObject
