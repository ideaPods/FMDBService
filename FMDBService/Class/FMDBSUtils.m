...
//  FMDBService
//
//  Created by William.Qiu on 2017/1/9.
//  Copyright (c) 2013年 TVM. All rights reserved.
//
...
@interface FMNumberFormatter : NSNumberFormatter

@end

@implementation FMNumberFormatter
...
+ (NSDateFormatter *)getDBDateFormat
{
    ...
    dispatch_once(&onceToken, ^{
        format = [[FMDateFormatter alloc] init];
...
+ (NSNumberFormatter *)numberFormatter
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        numberFormatter = [[FMNumberFormatter alloc] init];
    });
    return numberFormatter;
}
@end

inline NSString *FMSQLTypeFromObjcType(NSString *objcType)
{
    if ([FMSQL_Convert_IntType rangeOfString:objcType].length > 0) {
        return FMSQL_Type_Int;
    }
    if ([FMSQL_Convert_FloatType rangeOfString:objcType].length > 0) {
        return FMSQL_Type_Double;
    }
    if ([FMSQL_Convert_BlobType rangeOfString:objcType].length > 0) {
        return FMSQL_Type_Blob;
    }

    return FMSQL_Type_Text;
}

@implementation FMDBSQueryParams
